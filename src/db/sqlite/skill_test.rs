...
async fn skill_delete_invalidates_cache() {
    ...
    let skill_name = "delete-cache-test";
    let skill_id = crate::skills::generate_skill_id(skill_name);
...
"#
    ...
    let attachment = SkillAttachment {
        ...
        filename: "references/guide.md".to_string(),
    ...
    // Use temp directory for cache in tests
    let temp_cache =
        std::env::temp_dir().join(format!("test-skill-cache-delete-{}", std::process::id()));
    ...
    let cache_dir = skills::extract_attachments(&skill_id, &attachments, Some(temp_cache))
    ...
    let attachments_after = repo.get_attachments(&skill_id).await;
    assert!(
