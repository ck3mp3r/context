...
/// How a function/method call was made — determines resolution strategy.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum CallForm {
    ...
    Method,
    /// Scoped/qualified call: `Foo::bar()` (Rust) or `pkg.Func()` (Go)
    Scoped,
}

/// A symbol extracted from source code, before graph insertion.
#[derive(Debug, Clone)]
...
pub struct RawCall {
    ...
    /// How the call was made (free, method, scoped)
    pub call_form: CallForm,
    /// For method calls: the receiver expression text (e.g., "self", "obj")
    pub receiver: Option<String>,
    /// For scoped calls: the qualifier (e.g., "Foo" in `Foo::bar()`, "pkg" in `pkg.Func()`)
    pub qualifier: Option<String>,
    /// The enclosing symbol where this call occurs (set during symbol-call association)
    pub enclosing_symbol_idx: Option<usize>,
}

/// An import/use statement extracted from source code.
/// Reuses the existing ImportEntry type for the import data itself,
/// adding file context.
#[derive(Debug, Clone)]
...
/// An inheritance/implementation relationship extracted from source code.
#[derive(Debug, Clone)]
...
/// A containment relationship (parent contains child symbol).
#[derive(Debug, Clone)]
pub struct RawContainment {
    pub file_path: String,
    /// Parent symbol name (type, module, package)
    pub parent_name: String,
    /// Child symbol index in the ParsedFile's symbols vec
    pub child_symbol_idx: usize,
}

/// A type reference from one symbol to another.
#[derive(Debug, Clone)]
pub struct RawTypeRef {
    pub file_path: String,
    /// Index of the source symbol in ParsedFile's symbols vec
...
/// Produced by query-based extraction, consumed by resolution pipeline.
#[derive(Debug)]
...
/// An import/use statement with file context.
#[derive(Debug, Clone)]
pub struct RawImport {
    pub file_path: String,
    pub entry: ImportEntry,
