...
pub struct ListTaskListsParams {
    #[schemars(
        description = "FTS5 search query (optional). If provided, performs full-text search. Searches title, description, notes, tags, external_refs. Examples: 'rust backend' (simple), 'rust AND backend' (Boolean), '\"exact phrase\"' (phrase match)"
    )]
...
impl<D: Database + 'static> TaskListTools<D> {
    ...
    pub async fn list_task_lists(
        ...
        let result = if let Some(q) = &params.0.query {
            self.db.task_lists().search(q, Some(&query)).await
        } else {
            self.db.task_lists().list(Some(&query)).await
        }
