...
use context_core::{
    Database, HasProjects, HasTaskLists, HasTasks, ProjectRepository, Task, TaskList,
...
/// Test valid task transitions that don't set timestamps (simple status changes)
#[tokio::test(flavor = "multi_thread")]
async fn test_transitions_without_timestamp_changes() {
    let scenarios = vec![
        // (from_status, to_status, description)
        (TaskStatus::Backlog, "todo", "Backlog → Todo"),
        (TaskStatus::Todo, "backlog", "Todo → Backlog"),
    ];
    ...
    for (from_status, to_status, description) in scenarios {
        ...
        let expected_status = match to_status {
            "todo" => TaskStatus::Todo,
            "backlog" => TaskStatus::Backlog,
            _ => panic!("Unexpected status: {}", to_status),
        };
        assert_eq!(
...
#[tokio::test(flavor = "multi_thread")]
async fn test_transitions_from_cancelled() {
    let scenarios = vec![
        // (to_status, expected_status, description)
        ("todo", TaskStatus::Todo, "Cancelled → Todo"),
        (
            "in_progress",
...
#[tokio::test(flavor = "multi_thread")]
async fn test_create_task_with_jira_external_ref() {
    let db = SqliteDatabase::in_memory().await.unwrap();
    db.migrate().unwrap();
    let db = Arc::new(db);
    let tools = TaskTools::new(db.clone(), ChangeNotifier::new());
    ...
    // Create task with Jira external_ref
    let create_params = CreateTaskParams {
        list_id: created_list.id.clone(),
        title: "Implement Jira ticket".to_string(),
        description: None,
        priority: None,
        parent_id: None,
        tags: None,
...
#[tokio::test(flavor = "multi_thread")]
async fn test_create_task_without_external_ref() {
    let db = SqliteDatabase::in_memory().await.unwrap();
    db.migrate().unwrap();
    let db = Arc::new(db);
    let tools = TaskTools::new(db.clone(), ChangeNotifier::new());
    ...
    let create_params = CreateTaskParams {
        list_id: created_list.id.clone(),
        title: "Task without external ref".to_string(),
        description: None,
        priority: None,
        parent_id: None,
        tags: None,
...
#[tokio::test(flavor = "multi_thread")]
async fn test_update_task_external_ref() {
    let db = SqliteDatabase::in_memory().await.unwrap();
    db.migrate().unwrap();
    let db = Arc::new(db);
...
#[tokio::test(flavor = "multi_thread")]
async fn test_get_task_returns_external_ref() {
    let db = SqliteDatabase::in_memory().await.unwrap();
    db.migrate().unwrap();
    let db = Arc::new(db);
